...
def run_comparison():
    ...
    # 2. Build a lookup dictionary for the theoretical data
    # Format: theo_lookup["base_name__num_qubits"][variant_id] = { ... variant data ... }
    theo_lookup = {}
    ...
    # 3. Match data and extract lists for correlation
    all_tot_depth = []
    all_2q_depth = []
    
    all_emp_fid = []
    all_th_raw = []
    all_th_eff = []  # The "made up" / normalized effective fidelity
    ...
    # 4. Calculate Correlations (Pearson = linear fit, Spearman = monotonic/rank-based fit)
    ...
    # 5. Save Merged JSON (original, unchanged, for backward compatibility)
    merged_data = {
    ...
    # 5b. Save a NEW JSON file that includes both Pearson and Spearman side by side
    ...
    # 6. Generate Comparison Plots (now show both r and rho in titles)
    plt.figure(figsize=(14, 10))

    # Plot A: Total Depth vs Empirical Fidelity
    plt.subplot(2, 2, 1)
    ...
    # Plot B: 2Q Depth vs Empirical Fidelity
    plt.subplot(2, 2, 2)
    ...
    # Plot C: Theoretical (Raw) vs Empirical
    plt.subplot(2, 2, 3)
    ...
    # Plot D: Theoretical (Effective) vs Empirical
    plt.subplot(2, 2, 4)
